<x-app-layout>
    ...
    <div class="py-12">
        <!-- Comprobamos si esta logeado para que muestre el boton de añadir -->
        @if (Auth::check())
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <x-primary-button class="mt-4" onclick="openModalAddArticle()">
                    {{ __('Add article') }}
                </x-primary-button>
            </div>
        @endif
            ...
            <div class="mt-6 bg-white dark:bg-gray-800 shadow-sm rounded-lg divide-y dark:divide-gray-900">
                @foreach ($articles as $article)
                    <div class="p-6 flex space-x-2">
                        ...
                        @can('update', $article)
                            <div>
                                <x-secondary-button class="mt-4" onclick="openModalUpdateArticle()">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                        stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                            ...
                            <form method="POST" action="{{ route('articles.destroy', $article) }}">
                                @csrf @method('DELETE')
                                <x-danger-button class="mt-4" type="submit">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                        stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                        <path stroke-linecap="round" stroke-linejoin="round"
